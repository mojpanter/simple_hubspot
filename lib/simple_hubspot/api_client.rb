  ...
  class ApiClient
    class << self
      ...
      def do_post(path = nil, params = {}, headers = {})
        response = RestClient.post "#{SimpleHubspot.configuration.api_base}#{path}", params.to_json, { Authorization: "Bearer #{SimpleHubspot.configuration.hapikey}", content_type: :json }
      ...
      def do_get(path = nil, params = {}, headers = {})
        response = RestClient.get "#{SimpleHubspot.configuration.api_base}#{path}", { Authorization: "Bearer #{SimpleHubspot.configuration.hapikey}" }
